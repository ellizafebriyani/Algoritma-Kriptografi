...
def download_text_file(result, filename="hasil.txt"):
    buffer = BytesIO()
...
def view_vigenere_result():
    ...
    if request.form["type-out"] == "file":
        return download_text_file(result, act + ".txt")
...
def view_fullvigenere_result():
    ...
    if output_type == "file":
        return download_text_file(result, act + "_result.txt")
...
def view_extvigenere_result():
    ...
    if request.form["type-inp"] == "txt":
        ...
        f_loc = os.path.join("/tmp", f.filename)
        f.save(f_loc)
